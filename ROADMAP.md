...
> **Testing, honestly:** `pnpm test` runs **190 tests** (`node:test` via `tsx`, no new
...
> beta checks also cover SES-recipient policy, authenticated beta sender ownership,
> and idempotent conversation recording with history-preserving display repair. The
...
3. **Proof & compliance as the wedge no competitor has.** Layer-3 signed proof bundles
   ...
   *Phase 2 shipped:* **data-retention policies** — per-workspace window (`GET`/`PUT
   ...
   rejected). Data residency + a SOC 2 readiness map also shipped (above). *Shipped
   (2026-07-04):* **SCIM 2.0 provisioning** — `/scim/v2/Users` (list/get/create/replace/
   patch/delete) + ServiceProviderConfig, per-org bearer token, JIT create + immediate
   deprovision (an inactive membership grants no access — enforced in `userWorkspaces` /
