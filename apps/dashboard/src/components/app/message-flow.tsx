...
// Compact per-row lifecycle: four dots (Queued → Sent → Delivered → Opened) plus
// the furthest thing that actually happened, read from engagement timestamps —
// the stored status caps at "delivered", so an opened email would otherwise
// still read "delivered" in every list. Problem states keep their loud badge.
...
function furthest(m: FlowInput): { stage: number; label: string } {
  if (m.clicked_at) return { stage: 4, label: "Clicked" };
...
export function MessageFlow({ message }: { message: FlowInput }) {
  ...
  const title = `Queued → Sent → Delivered → Opened${message.clicked_at ? " → Clicked" : ""} · ${label}`;
  return (
