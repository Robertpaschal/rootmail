  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-muted px-4 py-10">
      <div className="w-full max-w-md">
        {!info ? (
          <div className="rounded-lg border border-rule bg-white p-8 text-center shadow-sm">
            <h1 className="text-lg font-semibold text-foreground">This page isn't available</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              This signup link is no longer active. If someone sent it to you, let them know.
            </p>
          </div>
        ) : (
          <div className="rounded-lg border border-rule bg-white p-8 text-foreground shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{info.org_name}</p>
            ...
            {state === "confirm_sent" ? (
              <div className="text-center">
                <MailCheck className="mx-auto mb-3 size-10 text-witnessed" />
                <h2 className="text-lg font-semibold">Check your email</h2>
                <p className="mt-1 text-sm text-muted-foreground">Click the confirmation link we just sent and you're on the list.</p>
              </div>
            ) : state === "subscribed" || state === "waitlisted" ? (
              <div className="text-center">
                <CheckCircle2 className="mx-auto mb-3 size-10 text-witnessed" />
        ...
        <p className="mt-4 text-center text-xs text-muted-foreground">
          Powered by <span className="font-medium text-muted-foreground">rootmail</span>
        </p>
      </div>
    </div>
  );
